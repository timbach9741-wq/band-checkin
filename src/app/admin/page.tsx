          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div className={`p-6 ${isRewardSuccess ? 'bg-yellow-50/50' : 'bg-slate-50'}`}>
              <div className="flex items-center gap-2 mb-4 pb-4 border-b border-slate-200/50">
                ...
                <h2 className="text-lg font-bold text-slate-800">방장 혜택 달성률 (오늘의 출석체크)</h2>
              </div>
              <h3 className={`text-xl font-black mb-2 ${isRewardSuccess ? 'text-yellow-800' : 'text-slate-700'}`}>{rewardTitle}</h3>
              <div className="flex items-center gap-4 mb-4">
                <div className="flex-1 bg-slate-200 h-4 rounded-full overflow-hidden">
                  <div className={`h-full transition-all duration-1000 ${isRewardSuccess ? 'bg-yellow-400' : 'bg-indigo-500'}`} style={{ width: `${Math.min(currentRate, 100)}%` }}></div>
                </div>
              ...
              {!isRewardSuccess && (
                <p className="text-xs mt-3 text-slate-400 font-medium leading-relaxed">※ 방 전체 인원({totalMembers}명)의 60% 이상이 20일 출석을 완료해야 방장님께 운영 지원용 기프티콘이 지급됩니다.</p>
              )}
            </div>
          </div>
        )}
