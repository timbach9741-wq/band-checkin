...
export default function Home() {
  ...
  const handleGenerateLink = async () => {
    ...
    // Removed regex validation to allow emojis, commas, and special characters in band names.
    // The backend API already sanitizes the bandName when creating the URL-safe bandId.
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-indigo-200">
      ...
      <main className="flex flex-col items-center">
        ...
        {/* 1.5. 실제 방장님들의 리얼 후기 (Social Proof - Marquee) */}
        <section className="w-full bg-slate-50/80 pt-16 pb-24 border-b border-slate-200 z-10 relative overflow-hidden">
                ...
                {displayReviews.map((review, idx) => (
                  <div key={idx} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/60 transition-all hover:shadow-md text-left w-[340px] shrink-0 whitespace-normal flex flex-col justify-between relative">
                    ...
                    <div>
                      ...
                      <div className="flex gap-1 mb-3">{'⭐⭐⭐⭐⭐'.split('').map((s,i)=><span key={i} className="text-yellow-400 text-xs">{s}</span>)}</div>
                      <p className="text-sm text-slate-600 font-medium leading-relaxed break-keep" dangerouslySetInnerHTML={{__html: review.text}}></p>
                    </div>
                  </div>
                ))}
              </div>
